...
// Higher deployment target than the other Swift sidecars: SCScreenshotManager
// is macOS 14+. Older systems simply don't get window OCR — the manager treats
// a missing/failing binary as "feature unavailable".
...
if (!swiftTarget) {
  console.error(`[window-ocr] Unsupported architecture: ${targetArch}`);
  process.exit(1);
}

const projectRoot = path.resolve(__dirname, "..");
const swiftSource = path.join(projectRoot, "resources", "macos-window-ocr.swift");
const outputDir = path.join(projectRoot, "resources", "bin");
const outputBinary = path.join(outputDir, "macos-window-ocr");
const hashFile = path.join(outputDir, `.macos-window-ocr.${targetArch}.hash`);
...
function log(message) {
  console.log(`[window-ocr] ${message}`);
}
...
if (!fs.existsSync(swiftSource)) {
  console.error(`[window-ocr] Swift source not found at ${swiftSource}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error("[window-ocr] Failed to compile macOS window OCR binary.");
...
try {
  ...
  console.warn(`[window-ocr] Unable to set executable permissions: ${error.message}`);
}

// Verify the compiled binary matches the target architecture
if (!verifyBinaryArch(outputBinary, targetArch)) {
  console.error(
    `[window-ocr] FATAL: Compiled binary architecture does not match target (${targetArch}). ` +
...
log(`Successfully built macOS window OCR binary (${targetArch}).`);

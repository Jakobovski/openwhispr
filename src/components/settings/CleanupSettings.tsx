...
export default function CleanupSettings() {
  ...
  const models = providers.find((provider) => provider.id === reconcileProvider)?.models ?? [];
  ...
  const handleProviderChange = (providerId: string) => {
    setReconcileProvider(providerId);
    const first = providers.find((provider) => provider.id === providerId)?.models[0];
    if (first) setReconcileModel(first.value);
  };
    ...
    <div className="space-y-4">
      ...
      <SettingsPanel>
        <SettingsPanelRow>
          ...
          >
            <Select value={reconcileProvider} onValueChange={handleProviderChange}>
        ...
        <SettingsPanelRow>
          <SettingsRow label={t("settingsPage.transcription.dualReconcileModel")}>
            <Select value={reconcileModel} onValueChange={setReconcileModel}>

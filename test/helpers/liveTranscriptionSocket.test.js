...
test("gemini: a partial replaces, and the final replaces the accumulator", async () => {
  const partials = [];
  const { wss, port } = await startServer((ws) => {
    ws.on("message", (data, isBinary) => {
      if (!isBinary && data.toString().includes('"setup"')) {
        ws.send(JSON.stringify({ setupComplete: {} }));
        // Gemini resends the whole utterance each time, so the consumer must not append.
        ws.send(JSON.stringify({ serverContent: { interimInputTranscription: { text: "Let's" } } }));
        ws.send(
          JSON.stringify({ serverContent: { interimInputTranscription: { text: "Let's ship" } } })
        );
        ws.send(
          JSON.stringify({
  ...
  assert.deepEqual(partials, ["Let's", "Let's ship"], "partials must not accumulate");
  assert.equal(result.text, "Let's ship the build.");
